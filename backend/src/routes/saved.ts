    ...
    try {
        const result = await pool.query(
            `SELECT id, type, item_id AS "itemId", title, summary, category,
                    icon, address, distance, rating, content,
                    saved_at AS "savedAt"
             FROM saved_items
             WHERE user_id = $1
...
router.post("/", async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { type, itemId, title, summary, category, icon, address, distance, rating, content } = req.body;
        ...
        const result = await pool.query(
            `INSERT INTO saved_items
                (user_id, type, item_id, title, summary, category, icon, address, distance, rating, content)
             ...
             RETURNING id, type, item_id AS "itemId", title, summary, category,
                       icon, address, distance, rating, content, saved_at AS "savedAt"`,
            [
                req.user!.userId, type, itemId, title,
                summary  ?? null, category ?? null,
                icon     ?? null, address  ?? null,
