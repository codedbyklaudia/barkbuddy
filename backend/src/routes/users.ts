  ...
  try {
    ...
    const result = await pool.query(
      `SELECT
        ...
        (SELECT name       FROM dogs WHERE user_id = u.id ORDER BY is_main DESC, created_at ASC LIMIT 1) AS "dogName",
        (SELECT breed      FROM dogs WHERE user_id = u.id ORDER BY is_main DESC, created_at ASC LIMIT 1) AS "dogBreed",
        (SELECT avatar_url FROM dogs WHERE user_id = u.id ORDER BY is_main DESC, created_at ASC LIMIT 1) AS "dogAvatarUrl",
        (SELECT life_stage FROM dogs WHERE user_id = u.id ORDER BY is_main DESC, created_at ASC LIMIT 1) AS "dogLifeStage"
      FROM users u
      WHERE u.id != $1
      AND (
        LOWER(u.name) LIKE $2
        OR EXISTS (SELECT 1 FROM dogs d WHERE d.user_id = u.id AND LOWER(d.name) LIKE $2)
      )
      ORDER BY u.name ASC
      LIMIT 20`,
