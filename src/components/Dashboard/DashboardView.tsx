...
import {
  ...
  BarChart2, CheckCircle2, ChevronLeft, Bone, Dumbbell, Camera,
} from "lucide-react";
import {
...
interface UserProfile {
  ...
  profileComplete: number; avatarUrl?: string; bannerUrl?: string;
...
interface PublicProfile {
  id: string; name: string; bio?: string; avatarUrl?: string; bannerUrl?: string;
  createdAt: string; buddyCount: number; postCount: number; 
  dog?: DogSummary;
  dogs?: DogSummary[];
}
...
// Banner upload API
async function uploadUserBanner(token: string, file: File): Promise<{ bannerUrl: string }> {
...
const CARE_ITEMS_CONFIG: {
  key: keyof CareLog; Icon: React.FC<any>; label: string; desc: string; color: string;
}[] = [
...
const DOG_ACCENTS = ["#6d4fc2", "#9b72d8", "#4a3870", "#b89ee8", "#7c5cbf"] as const;
...
// Buddy helpers
function buddyDogLabel(b: Buddy): string | null {
  if (b.dogs && b.dogs.length > 0) return b.dogs.map(d => d.name).join(", ");
  return b.dogName ?? null;
}

function deduplicateBuddies(raw: Buddy[]): Buddy[] {
  const map: Record<string, Buddy> = {};
  for (const buddy of raw) {
    if (!map[buddy.userId]) {
      map[buddy.userId] = {
        ...buddy,
        dogs: buddy.dogName ? [{ name: buddy.dogName, breed: buddy.dogBreed }] : [],
      };
    } else if (buddy.dogName) {
      const existing = map[buddy.userId].dogs ?? [];
      if (!existing.some(d => d.name === buddy.dogName)) {
        map[buddy.userId].dogs = [...existing, { name: buddy.dogName, breed: buddy.dogBreed }];
      }
    }
  }
    ...
    <div className="streak-celebration-overlay" onClick={onClose}>
      <div className="streak-celebration-card" onClick={e => e.stopPropagation()}>
        ...
        <p className="streak-cel-sub">
          {streak === 1 ? "You started your streak! Keep going!" :
...
const ProgressRing: React.FC<{ done: number; total: number; color: string }> = ({ done, total, color }) => {
  ...
  return (
    <div className="checkin-ring-wrap">
      ...
      <div className={`checkin-ring-label${isDone ? " ring-complete" : ""}`} style={{ color: isDone ? "#9b72d8" : color }}>
        {done}/{total}
      </div>
    </div>
  );
...
interface MultiDogCheckInModalProps {
  allDogs: DogProfile[]; checkins: string[];
  dogLogs: Record<string, Record<string, CareLog>>;
  onConfirm: (updatedDogLogs: Record<string, Record<string, CareLog>>) => void;
  onClose: () => void;
}

const CheckInModal: React.FC<MultiDogCheckInModalProps> = ({ allDogs, checkins, dogLogs, onConfirm, onClose }) => {
  ...
  const [localLogs, setLocalLogs] = useState<Record<string, CareLog>>(() => {
    ...
    allDogs.forEach(dog => { init[dog.id] = { ...(dogLogs[dog.id]?.[todayKey] ?? EMPTY_LOG) }; });
    return init;
  });
  ...
  const dogDoneCount = (dogId: string) => LOG_KEYS.filter(k => localLogs[dogId]?.[k]).length;
  ...
  const toggleItem = (key: keyof CareLog) => {
    setLocalLogs(prev => ({ ...prev, [activeDog.id]: { ...prev[activeDog.id], [key]: !prev[activeDog.id][key] } }));
  };

  const handleConfirm = () => {
    const updated: Record<string, Record<string, CareLog>> = { ...dogLogs };
    allDogs.forEach(dog => { updated[dog.id] = { ...(updated[dog.id] ?? {}), [todayKey]: localLogs[dog.id] }; });
    onConfirm(updated);
    onClose();
  };
  ...
  return ReactDOM.createPortal(
    <div className="modal-overlay" onClick={onClose}>
      <div className="checkin-modal" onClick={e => e.stopPropagation()} role="dialog" aria-modal aria-label="Daily Check-In"
        ...
        <div className="checkin-topbar">
          ...
          <button className="checkin-close" onClick={onClose} aria-label="Close check-in"><X size={16} /></button>
        </div>
        ...
        {allDogs.length > 1 && (
          <div className="checkin-switcher" role="tablist" aria-label="Select dog">
            {allDogs.map((dog, idx) => {
              ...
              return (
                <button key={dog.id} className={`checkin-chip${isActive ? " active" : ""}${isDone ? " chip-done" : ""}`}
                  style={{ "--chip-color": color } as React.CSSProperties}
                  onClick={() => setActiveDogIdx(idx)} role="tab" aria-selected={isActive}
                  ...
                  <span className="checkin-chip-progress">{isDone ? "✓" : `${done}/${LOG_KEYS.length}`}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="checkin-body">
          <div className="checkin-dog-hero" style={{ "--hero-color": accentColor } as React.CSSProperties}>
            <div className="checkin-dog-hero-avatar" style={{ background: accentColor }}>
              {activeDog.avatarUrl ? <img src={activeDog.avatarUrl} alt={activeDog.name} /> : <Dog size={22} strokeWidth={1.6} />}
            </div>
            <div className="checkin-dog-hero-text"><div className="checkin-dog-hero-name">{activeDog.name}</div></div>
          ...
          <div className="checkin-items">
            {CARE_ITEMS_CONFIG.map((item, i) => {
              ...
              const { Icon } = item;
              return (
                <button key={item.key} className={`checkin-item${isDone ? " item-done" : ""}`}
                  onClick={() => toggleItem(item.key)} aria-pressed={isDone}
                  style={{ "--item-color": item.color, "--item-delay": `${i * 0.055}s` } as React.CSSProperties}>
                  <div className="checkin-item-icon"><Icon size={22} strokeWidth={1.8} /></div>
                  ...
                  <div className="checkin-item-check" aria-hidden>{isDone && <Check size={13} strokeWidth={3} />}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="checkin-footer">
          <div className="checkin-streak-pill"><Flame size={14} /><span>{streak}</span></div>
          <div className="checkin-footer-progress">
            <div className="checkin-footer-track"><div className="checkin-footer-fill" style={{ width: `${overallPct}%` }} /></div>
          ...
          {allDogs.length > 1 && activeDogIdx < allDogs.length - 1 ? (
            <button className="checkin-next-btn" onClick={() => setActiveDogIdx(i => i + 1)}
              style={{ "--accent-color": accentColor } as React.CSSProperties}>
              {allDogs[activeDogIdx + 1].name} <ChevronRight size={14} />
            </button>
          ) : (
            <button className={`checkin-save-btn${overallPct === 100 ? " btn-glow" : ""}`} onClick={handleConfirm}
...
const BuddiesPanel: React.FC<{ token: string }> = ({ token }) => {
  ...
  useEffect(() => {
    ...
    getBuddies(token).then(({ buddies: b, pendingIn: pi, pendingOut: po }) => {
      setBuddies(deduplicateBuddies(b));
      setPendingIn(pi);
      setPendingOut(po);
...
const MyPostsPanel: React.FC<{ token: string; onViewInForum: (postId: string) => void }> = ({ token, onViewInForum }) => {
  ...
  useEffect(() => { setLoading(true); getMyPosts(token).then(setPosts).finally(() => setLoading(false)); }, [token]);
  ...
  if (posts.length === 0) return (
    <div className="posts-empty">
      ...
      <p>No posts yet</p><p className="empty-sub">Head to the Community Forum to share your first post!</p>
    </div>
  );

  return (
    <div className="posts-panel">
      <div className="posts-panel-header"><span className="posts-count-chip">{posts.length} post{posts.length !== 1 ? "s" : ""}</span></div>
          ...
          <div key={post.id} className={`post-card ${!post.isPublished ? "is-draft" : ""}`}>
            ...
            <div className="post-card-body">
              ...
              <div className="post-card-actions">
                <button className="post-action view" onClick={() => onViewInForum(post.id)} title="View in Forum"><Eye size={15} /></button>
                <button className={`post-action ${post.isPublished ? "hide" : "publish"}`} onClick={() => handleTogglePublish(post)}
                  disabled={toggling === post.id} title={post.isPublished ? "Hide" : "Publish"}>
...
const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  onBannerUpload, bannerUploading,
}) => {
  ...
  const buddySinceDate = buddySince
    ...
    : null;
  const allProfileDogs: DogSummary[] = profile.dogs?.length ? profile.dogs : profile.dog ? [profile.dog] : [];
  const getDogAge = (dob?: string) => dob ? Math.floor((Date.now() - new Date(dob).getTime()) / (1000 * 60 * 60 * 24 * 365)) : null;
  ...
  <div className="pc-banner">
    {profile.bannerUrl
      ? <img src={profile.bannerUrl} alt="Profile banner" className="pc-banner-img" />
      : <div className="pc-banner-bg" />}

    {isSelf && onBannerUpload && (
      <button
        className="pc-banner-upload-btn"
        onClick={() => bannerInputRef.current?.click()}
        disabled={bannerUploading}
        title={profile.bannerUrl ? "Change banner" : "Add banner photo"}
        aria-label={profile.bannerUrl ? "Change banner" : "Add banner photo"}
      >
        {bannerUploading
          ? <Loader2 size={13} className="spin" />
          : <><Camera size={13} /> {profile.bannerUrl ? "Change banner" : "Add banner"}</>}
      </button>
    )}
  </div>

  {/* Avatar — outside pc-banner so it never gets clipped */}
  <div className="pc-avatar-wrap">
    ...
    {streak >= 3 && <div className="pc-streak-pip"><Flame size={11} /><span>{streak}</span></div>}
  </div>

  {/* Hidden file input — outside pc-banner so overflow:hidden can't block it */}
  {isSelf && onBannerUpload && (
    <input
      ref={bannerInputRef}
      type="file"
      accept="image/*"
      style={{ display: "none" }}
      onChange={e => { if (e.target.files?.[0]) onBannerUpload(e.target.files[0]); }}
    />
  )}
            ...
            return (
              <div key={i} className="pc-dog-card">
                <div className="pc-dog-avatar">
                  {d.avatarUrl ? <img src={d.avatarUrl} alt={d.name} /> : <Dog size={22} strokeWidth={1.4} />}
                </div>
                <div className="pc-dog-info">
                  ...
                  <span className="pc-dog-breed">{d.breed}{age !== null ? ` · ${age}y old` : ""}{" · "}{d.gender}</span>
                  {d.personality && d.personality.length > 0 && (
                    <div className="pc-dog-traits">
                      {d.personality.slice(0, 4).map(t => (<span key={t} className="pc-dog-trait">{t.replace(/_/g, " ")}</span>))}
                    </div>
                  )}
                </div>
              </div>
      ...
      {isSelf && profileComplete !== undefined && profileComplete < 100 && (
        <div className="pc-complete">
          ...
          <div className="pc-complete-bar"><div className="pc-complete-fill" style={{ width: `${profileComplete}%` }} /></div>
      ...
      {!isSelf && onRemoveBuddy && (
        <button className="pc-remove-btn" onClick={onRemoveBuddy} disabled={removingBuddy}>
          {removingBuddy ? <><Loader2 size={14} className="spin" /> Removing…</> : <><UserMinus size={14} /> Remove Buddy</>}
        </button>
      )}
    </div>
  );
...
const ProfileCardSkeleton: React.FC = () => (
  <div className="pc-root pc-skeleton">
    ...
    <div className="pc-identity"><div className="skel skel-name" /><div className="skel skel-bio" /></div>
...
const PublicProfilePreview: React.FC<{
  user: UserProfile; dog: DogProfile | null; allDogs: DogProfile[];
  checkins: string[]; token: string;
  onBannerUpdate: (bannerUrl: string) => void;
}> = ({ user, dog, allDogs, checkins, token, onBannerUpdate }) => {
  const [profile, setProfile]                 = useState<PublicProfile | null>(null);
  const [loading, setLoading]                 = useState(true);
  ...
  const handleBannerUpload = async (file: File) => {
    ...
    try {
      const { bannerUrl } = await uploadUserBanner(token, file);
  ...
  const fallback: PublicProfile = {
    ...
    buddyCount: 0, postCount: 0, 
    dogs: allDogs.map(d => ({
      name: d.name, breed: d.breed, lifeStage: d.lifeStage,
      gender: d.gender, dob: d.dob, avatarUrl: d.avatarUrl,
      personality: d.personality,
    })),
  };

  const displayProfile: PublicProfile = profile
    ? { ...profile, bannerUrl: profile.bannerUrl ?? user.bannerUrl }
    : fallback;
...
const MyPostsBentoCard: React.FC<{
  ...
  useEffect(() => { getMyPosts(token).then(setPosts).finally(() => setLoading(false)); }, [token]);
    ...
    <div className="dv-card dv-card--myposts">
      ...
      {!loading && posts.length === 0 && (
        <div className="myposts-empty">
          <MessageCircle size={28} strokeWidth={1.2} /><p>No posts yet</p>
            ...
            <button key={post.id} className="myposts-row" onClick={() => onViewPost(post.id)}>
              <div className="myposts-row-main">
                ...
                <div className="myposts-row-meta">
                  <span className={`myposts-status ${post.isPublished ? "published" : "draft"}`}>{post.isPublished ? "Published" : "Draft"}</span>
                  <span className="myposts-row-stats"><Heart size={11} /> {post.likesCount}<MessageCircle size={11} /> {post.commentsCount}</span>
                </div>
              </div>
      ...
      {!loading && posts.length > 3 && <button className="myposts-more" onClick={onViewForum}>+{posts.length - 3} more posts</button>}
    </div>
  );
};
...
interface DashboardViewProps {
  user: UserProfile; dog: DogProfile | null; allDogs: DogProfile[];
  token: string; onNav: (key: string) => void;
...
const DashboardView: React.FC<DashboardViewProps> = ({ user, dog, allDogs, token, onNav, onViewForumPost, onUserUpdate }) => {
  const todayKey = getTodayKey();
  ...
  const [dogLogs, setDogLogs] = useState<Record<string, Record<string, CareLog>>>(() => {
    ...
    if (dog) {
      ...
      if (Object.keys(legacy).length > 0 && !Object.keys(result[dog.id] ?? {}).length) result[dog.id] = legacy;
    }
    return result;
  });
  ...
  const dogsFullyDone = allDogs.filter(d => {
    const log = dogLogs[d.id]?.[todayKey] ?? EMPTY_LOG;  
  ...
  const handleCheckInConfirm = (updatedDogLogs: Record<string, Record<string, CareLog>>) => {
    allDogs.forEach(d => { if (updatedDogLogs[d.id]) saveDogLog(d.id, updatedDogLogs[d.id]); });
  ...
  return (
    <div className="dv-root">
      ...
      <div className="dv-hero">
        <div className="dv-hero-inner">
          <div className="dv-hero-text">
            ...
            <p className="dv-hero-sub">
              {allDogsCheckedIn
                ? allDogs.length > 1 ? "All your dogs are checked in for today 🐾" : `${dog?.name ?? "Your dog"} is all checked in for today.`
                : allDogs.length > 1 ? `Time for your pack's daily check-in (${dogsFullyDone.length}/${allDogs.length} done)`
        ...
        <button className={`dv-hero-checkin-btn ${allDogsCheckedIn ? "checked" : ""}`} onClick={() => setCheckInOpen(true)}>
          {allDogsCheckedIn
            ? <><Check size={16} /> Checked In</>
            : <><Bone size={16} />{allDogs.length > 1 ? `Pack Check-In (${dogsFullyDone.length}/${allDogs.length})` : `${dog?.name ?? "Buddy"} Check-In`}</>}
        </button>
      </div>
        ...
        <div className="dv-card dv-card--links">
          ...
          <div className="links-list">
            {quickLinks.map(link => {
              const { Icon } = link;
              return (
                <button key={link.label} className="link-row"
      ...
      <div className="dv-hub">
        <div className="dv-hub-tabs" role="tablist">
          {tabs.map(tab => {
            const { Icon } = tab;
            return (
              <button key={tab.key} className={`dv-hub-tab ${profileTab === tab.key ? "active" : ""}`}
                role="tab" aria-selected={profileTab === tab.key}
        ...
        <div className="dv-hub-panel" role="tabpanel">
          {profileTab === "preview" && (
            <PublicProfilePreview
              user={user} dog={dog} allDogs={allDogs} checkins={checkins} token={token}
              onBannerUpdate={(bannerUrl) => onUserUpdate?.({ bannerUrl })}
            />
          )}
      ...
      {checkInOpen && allDogs.length > 0 && (
        <CheckInModal allDogs={allDogs} checkins={checkins} dogLogs={dogLogs}
          onConfirm={handleCheckInConfirm} onClose={() => setCheckInOpen(false)} />
      )}
