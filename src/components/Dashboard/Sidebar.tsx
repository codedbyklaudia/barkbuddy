...
function IconDog({ size = 20 }: { size?: number }) {
  return <img src="./../images/icons/dog_icon.svg" width={size} height={size} alt="Dog" />;
}
...
export const MobileTopNav: React.FC<{
  ...
  userEmail?:  string;
  savedCount?: number;
}> = ({ open, active, onNav, onClose, onLogout, userAvatar, userName, userEmail, savedCount = 0 }) => {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`mob-nav-backdrop ${open ? "is-open" : ""}`}
        onClick={onClose}
        aria-hidden="true"
      />
      ...
      {/* Dropdown panel */}
      <div
        className={`mob-nav-panel ${open ? "is-open" : ""}`}
        role="dialog"
      ...
      >
        ...
        <div className="mob-nav-user">
          ...
          <div className="mob-nav-user-info">
            <span className="mob-nav-user-name">{userName ?? "My Account"}</span>
            {userEmail && <span className="mob-nav-user-email">{userEmail}</span>}
          </div>
          <button className="mob-nav-close" onClick={onClose} aria-label="Close menu">
            <IconClose size={18} />
          </button>
        </div>

        {/* Nav items */}
        <nav className="mob-nav-list" aria-label="Dashboard navigation">
          {NAV_ITEMS.map((item, i) => {
            ...
            return (
              <button
                key={item.key}
                className={`mob-nav-item ${isActive ? "is-active" : ""}`}
                style={{ animationDelay: `${i * 0.04}s` }}
                onClick={() => { onNav(item.key); onClose(); }}
                aria-current={isActive ? "page" : undefined}
              >
                <span className="mob-nav-item-icon">
                  <NavIcon size={20} />
                </span>
                <span className="mob-nav-item-label">{item.label}</span>
                {showBadge && (
                  <span className="mob-nav-item-badge">{savedCount}</span>
                )}
                {isActive && <span className="mob-nav-item-pip" aria-hidden="true" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="mob-nav-footer">
          <button
            className="mob-nav-footer-btn"
            onClick={() => { navigate("/"); onClose(); }}
          >
            <IconPlatform size={17} />
            Go to BarkBuddy
          </button>
          <button
            className="mob-nav-footer-btn mob-nav-footer-btn--logout"
