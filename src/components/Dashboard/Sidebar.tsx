...
const NAV_ITEMS = [
  ...
  { key: "forum",    label: "Forum",           icon: IconForum    },
...
function IconDog({ size = 20 }: { size?: number }) {
  return <Bone width={size} height={size} />;
}
...
function IconForum({ size = 20 }: { size?: number }) {
...
export const Sidebar: React.FC<{
  ...
  userAvatar?: string;
  userName?: string;
}> = ({ active, onNav, onLogout, userAvatar, userName }) => {
  ...
  return (
    ...
    >
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              title={item.label}
            >
              <span className="db-nav-icon"><NavIcon size={19} /></span>
      ...
      >
        ...
        <span className="db-nav-label">Go to BarkBuddy</span>
      </button>
